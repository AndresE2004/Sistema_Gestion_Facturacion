...
class Pago(Base):
    """Modelo de pago realizado por suscriptor"""
    __tablename__ = "pagos"
    ...
    monto_efectivo = Column(Numeric(10, 2), nullable=True)
    ...
    __table_args__ = (
        ...
        CheckConstraint('anio >= 2000', name='check_anio_valido'),
...
class Recibo(Base):
    """Modelo de recibo generado automáticamente"""
    __tablename__ = "recibos"

    id = Column(Integer, primary_key=True, index=True)
    pago_id = Column(Integer, ForeignKey("pagos.id", ondelete="CASCADE"), nullable=False, unique=True, index=True)
...
class Ingreso(Base):
    """Modelo de ingreso registrado automáticamente desde pagos"""
...
class Gasto(Base):
    ...
    __table_args__ = (
        CheckConstraint('valor > 0', name='check_valor_gasto_positivo'),
    )
